...
As requisiçoes de login/cadastro ficaram muito bem articuladas, com varias prevençoes e validaçoes de possiveis erros do usuario na digitaçao dos respectivos formularios, como a exibiçao de um <p> em tempo real em cima dos inputs, quando o usuario não corresponde a regex que eu defini no script(voce pode acompanhar isso em "/frontend/utils/validation.js"). Os alerts sao exibidos funcionalmente, em eventos esperados como "Senha incorreta", "usuario ja cadastrado", "etc". O CRUD basicamente é:
